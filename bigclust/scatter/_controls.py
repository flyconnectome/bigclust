...
class ScatterControls(QtWidgets.QWidget):
    ...
    def build_control_gui(self):
        ...
        for col in sorted(self.figure._data.columns):
            self.label_combo_box.addItem(col)
        self.label_layout.addWidget(self.label_combo_box)
        self.label_combo_box.currentIndexChanged.connect(self.set_point_labels)
    ...
    def build_settings_gui(self):
        ...
        self.font_size_label.setToolTip(
            "Set the font size for the labels in the figure."
        )
    ...
    def set_label_counts(self):
        ...
        self.set_point_labels()  # Update the labels
    ...
    def selected_to_clipboard(self, dataset=None):
        ...
        if self.figure.selected is not None:
            indices = self.figure._selected

            if isinstance(dataset, str):
                indices = [i for i in indices if self.figure._datasets[i] == dataset]
            elif isinstance(dataset, (list, set, tuple)):
                indices = [i for i in indices if self.figure._datasets[i] in dataset]
    ...
    try:
        ...
        if figure:
            ...
            if set_any_malecns and bodyids is not None:
                ...
                figure.set_point_labels(ind, f"{label}(!)")
    ...
    try:
        ...
        if figure:
            ...
            if (clear_flywire or clear_type or clear_manc_type) and bodyids is not None:
                ind = figure.selected[np.isin(figure.selected_ids, bodyids)]
                figure.set_point_labels(ind, "(cleared)(!)")
